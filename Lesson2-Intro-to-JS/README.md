...
Javascript has a number of built-in objects for us to use. We have already seen, and have been using, the console object and its method "log". Another of these objects is "Math". Math has a number of methods on it just like console has log. To add to this, some of our data types also have built-in methods.
...
Now that we have variables set we need functions to compute them, change them, (or ?) do something with them. There are three ways we can build a function.
